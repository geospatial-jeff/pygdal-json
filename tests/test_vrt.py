...
class VRTTestCases(unittest.TestCase):
    ...
    def translate(self, **kwargs):
        """Method to pass same args to gdal.Translate and gdaljson.Translate"""
    ...
    def warp(self, **kwargs):
        """Method to pass same args to gdal.Warp and gdaljson.Warp"""
    ...
    def check_equivalency(self, vrt1, vrt2):
        """Method to check if two VRTs have the same properties"""
        properties = [
            "xsize",
            "ysize",
            "tlx",
    ...
    def test_translate_resample(self):
        with self.open_vrt(self.translatevrt) as vrt:
            native, gdaljson = self.translate(
                xRes=vrt.xres / 2, yRes=vrt.yres / 2)
            self.check_equivalency(native, gdaljson)

            native, gdaljson = self.translate(
                xRes=vrt.xres / 2, yRes=vrt.yres / 2, resampleAlg="Cubic")
            self.check_equivalency(native, gdaljson)
    ...
    def test_warp_clipper(self):
        ...
        native, gdaljson = self.warp(
            clipper=clipper, cropToCutline=True, dstAlpha=True)
        self.check_equivalency(native, gdaljson)

        native, gdaljson = self.warp(
            clipper=clipper, cropToCutline=True, height=250)
        self.check_equivalency(native, gdaljson)

        native, gdaljson = self.warp(
            clipper=clipper, cropToCutline=True, height=250, width=150)
        self.check_equivalency(native, gdaljson)
        ...
        native, gdaljson = self.warp(
            clipper=clipper, dstSRS=3857, dstAlpha=True, height=100, width=100)
        self.check_equivalency(native, gdaljson)
