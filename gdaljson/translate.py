...
def translate(input_vrt, bandList=None, srcWin=None, projWin=None, height=None, width=None, xRes=None, yRes=None):
    ...
    if height or width:
        ...
        if height and width:
            ...
            if height:
                ...
                ratio = workingvrt.data['VRTDataset']['VRTRasterBand'][0][workingvrt.source]['SrcRect']['@ySize'] / height
                _width = int(workingvrt.data['VRTDataset']['VRTRasterBand'][0][workingvrt.source]['SrcRect']['@xSize']/ratio)
                _height = height
            elif width:
                #If just width is given, calculate matching height
                ratio = workingvrt.data['VRTDataset']['VRTRasterBand'][0][workingvrt.source]['SrcRect']['@xSize'] / width
                _height = int(workingvrt.data['VRTDataset']['VRTRasterBand'][0][workingvrt.source]['SrcRect']['@ySize']/ratio)
                _width = width
            workingvrt.update_dst_rect([0,0,_width,_height])
        #Calculate new resolution based on new dimensions
        gt[1] = (workingvrt.xres * workingvrt.data['VRTDataset']['VRTRasterBand'][0][workingvrt.source]['SrcRect']['@xSize']) / _width
        gt[5] = -(workingvrt.yres * workingvrt.data['VRTDataset']['VRTRasterBand'][0][workingvrt.source]['SrcRect']['@ySize']) / _height

    elif xRes and yRes:
        ratiox = workingvrt.xres * workingvrt.data['VRTDataset']['VRTRasterBand'][0][workingvrt.source]['SrcRect']['@xSize']
        ratioy = workingvrt.yres * workingvrt.data['VRTDataset']['VRTRasterBand'][0][workingvrt.source]['SrcRect']['@ySize']
    ...
    gt[0] = workingvrt.data['VRTDataset']['VRTRasterBand'][0][workingvrt.source]['SrcRect']['@xOff'] * workingvrt.xres + workingvrt.tlx
    gt[3] = workingvrt.tly - workingvrt.data['VRTDataset']['VRTRasterBand'][0][workingvrt.source]['SrcRect']['@yOff'] * workingvrt.yres
